...
A couple of weeks ago another great dataset was made available to the soccer analytics community. In collaboration with Friends-of-Tracking the data provider SkillCorner open-sourced tracking data for nine top5 league matches.
...
One of the downsides of broadcasting footage as a source is that it suffers from noise in the form of replays and close-up zooms. In this post I will try to introduce the format of the data and some ways to work-around the quirks of the data.

Let's start with loading the necessary libraries and with sourcing the data from SkillCorner's GitHub repository. I am also sourcing a custom pitch plotting function from one of my repositories. The format for the start and end time is HH:MM.(1/10th sec)
...
To plot the tracking data in an animation we first want to standardize the coordinates to a 105m x 68m norm to then use our plotting function. gganimate will then take care of the rest. Its function transition time specifies which dataframe column holds the frame transition information. We plot players and ball separately to ensure that the ball is always visible.
...
```{r Load Data and Set Parameters 2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
library(tidyverse) # Data manipulation
library(jsonlite) # Loading json files
library(gganimate) # Animation of plots
library(gridExtra) # Needed for adding tables to plot
...
```{r Extract tracking data 2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
# We are looping through frames and concatenate tracking data.
# If tracking data is not available we fill forward the previous
# frame and flag it as 'not in broadcast'.
...
```{r Interpolate missing tracking data 2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
# Below code interpolates linearly between locations
# for periods in which we are missing tracking data

coordinates <- coordinates %>%
  mutate(in_broadcast = ifelse(z > 0.1))
...
```{r Initial plot 2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
coordinates_int <- coordinates_int %>%
  left_join(players, by = "trackable_object" ) %>%
  mutate(team_id = ifelse(trackable_object == 55, 0, team_id)) %>%
  mutate(time = as.POSIXct(time, format="%M:%OS")) %>%
...
```{r Let make this look a bit nicer 2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
bay_img <- "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1b/FC_Bayern_M%C3%BCnchen_logo_%282017%29.svg/1024px-FC_Bayern_M%C3%BCnchen_logo_%282017%29.svg.png"
...
- SkillCorner [data repository](https://github.com/SkillCorner/opendata}
