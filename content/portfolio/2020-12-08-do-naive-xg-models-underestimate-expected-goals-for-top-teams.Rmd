...
The idea is to derive some kind of estimate for how similar they behave to a hypothetical, perfect xG model. For this analysis I will proxy the Naive model again with my own implementation which only relies on location based data.
...
This leaves us with 294 team seasons of data for number of shots, cumulative xG (for both Statsbomb and the Naive implementation) and goals. For a perfect model we expect cumulative goals to lie in a range of $\textstyle{+/- \sqrt{\dfrac{\#shots}{8}}}$ around cumulative xG for around 280 of these seasons (95% of 294). Any deviation from this may indicate the presence of modeling error in the models. 
...
We observe that the Statsbomb model behaves much more similar to the perfect model and that a very simple model is closer to an even simpler *No Model*.
...
We know that for a perfect xG model, 95% of cumulative goals lie within a certain range around cumulative xG. This drops only slightly for the Statsbomb data (~93%) but more drastic for the Naive model data (~81%). Which noise parameters (shift in the mean or level of variance) are consistent with such a behavior? 
...
dt <- datatable(res_wide, 
  ...
  formatStyle(c('0.001', '0.005', '0.01', '0.02', '0.03'), border = styleInterval(c(0.80, 0.82), c('auto', '2px solid #91bfdb', 'auto')))
...
df <- data.frame(
  x = 0.5,
  y = 4,
  label = "Distribution of likely, true xG values for <b style = 'color:#DC2228'>Statsbomb (bias: 0.005, variance: 0.005)</b> and <b style = 'color:#91bfdb'>Naive (bias: 0.02, variance: 0.01)</b> xG models"
)
...
Below I am plotting cumulative goals vs shots from our 294 team seasons. I am only only highlighting goal data that lies outside the 95% confidence intervals for both Naive and Statsbomb xG estimates. Cumulative goals that lie above the interval are highlighted red, goals below the interval are highlighted blue. As expected we see many more data points highlighted for the Naive data than for Statsbomb's.

In the case of the Naive Model, teams that outperform xG tend to be high volume shooters while teams that underperform are lower volume shooters. In other words the Naive model likely underestimates cumulative xG for teams with many shots and overestimates xG for teams with few shots. This mirrors the potential bias we already saw in our above analysis. Also consistent with above is that the sophisticated model does not show such a bias.
...
us <- ggplot(xg_df_us) +
  ...
  geom_point(aes(x = shots, y = xg_naive, alpha = in_range_us), color = "grey90") + 
...
us <- ggplot(xg_df_us) +
  ...
  geom_point(aes(x = elo, y = xg_naive, alpha = in_range_us), color = "grey90") + 
...
for(i in 1:500){
  ...
  xg_sample <- xg_sample %>%
    ...
    mutate(xg_naive = noise_sample(xg + 0.02, 0.01)) %>%
    ungroup()
