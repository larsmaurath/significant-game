...
Similar to the Barcelona chart above I show the number of shots on the x-axis and cumulative xG/goals on the y-axis. Possible cumulative goal paths are shown in grey around the red cumulative xG path. The second chart shows the one-sided, relative variation of the 95% confidence interval around cumulative xG.
...
With this rule of thumb we can now investigate how actual xG models compare to this. Deviations from this rule should give us some insight on their modeling errors.

### How do Statsbomb and Naive Models Perform Compared to a Perfect Model?
...
The idea is to derive some kind of estimate for how similar they behave to a hypothetical, perfect xG model. For this analysis I will proxy the Naive model with data from Understat.com which, based on the information they share on their website, should be quite similar to the model I implemented previously. 

#### Data

I am pulling xG data for the Top 5 leagues and three seasons (2017/2018, 2018/2019 and 2019/2020) from Statsbomb (via fbref.com) and understat.com. The history is limited by Statsbomb data while the league choice is limited by Understat.

This leaves us with 294 team seasons of data for number of shots, cumulative xG (for both Statsbomb and Understat) and goals. For a perfect model we expect cumulative goals to lie in a range of $\textstyle{+/- \sqrt{\dfrac{\#shots}{8}}}$ around cumulative xG for around 280 of these seasons (95% of 294). Any deviation from this may indicate the presence of modeling error in the models. 
...
The frequency of these *outliers* should give us an indication of the quality of the model. As we look at the performance of both models we will also include a benchmark *non-model* that simply uses the average shot xG (11.5%) for every shot.
...
How should we start thinking about these possible modeling errors? Let's assume we observe a shot and assign an xG value with our Naive model. Give that we are missing a lot of information (e.g. defender positioning) our result will be an average of all similar situations, no matter if the goal is empty or the 6 yard box is congested. Naive models try to overcome this with additional features like flags indicating that a shot occurs after a corner or fast break, but they can hardly be perfect.
...
We can also turn this thinking around: given a true xG value, poor models will generate a wider range of model estimates than great models. 
...
Now comes the biggest mental leap in this post: by simulating various mean and variance parameters for the beta distribution (the proxy for model noise) and their subsequent behavior in our first simulation I will try to back out the noise within the Statsbomb and Understat xG models.

We know that for a perfect xG model, 95% of cumulative goals lie within a certain range around cumulative xG. This drops only slightly for the Statsbomb data but more drastic for Understat. Which noise parameters (shift in the mean or level of variance) are consistent with such a behavior? 
...
Below I am plotting cumulative goals vs shots from our 294 team seasons. I am only only highlighting goal data that lies outside the 95% confidence intervals for both Understat and Statsbomb xG estimates. Cumulative goals that lie above the interval are highlighted red, goals below the interval are highlighted blue. As expected we see many more data points highlighted for the Understat data than for Statsbomb's.

In the case of Understat, teams that outperform xG tend to be high volume shooters while teams that underperform are lower volume shooters. In other words the Naive model likely underestimates cumulative xG for teams with many shots and overestimates xG for teams with few shots. This mirrors the potential bias we already saw in our above analysis. Also consistent with above is that the sophisticated model does not show such a bias.
...
My intuition that high shot volume is a proxy for high quality teams does not seem to hold. We do not observe the same biases when conditioning on Elo ratings. 
...
David answers two main questions: With what sample size does it make sense to start looking at xG values and when should you prefer actual goals over xG? In order to answer these questions I will initially deviate from David's guidance to again assume there is a hypothetical, perfect xG model who's output is superior even to goals at any sample size. Next I will measure noise to this hypothetical model for actual goals, a sophisticated model and a naive model. These two models are again proxied by the modeling errors I derived from Statsbomb and Understat data.
...
for(i in 1:500){
  ...
  xg_sample <- xg_sample %>%
    ...
    mutate(xg_naive = noise_sample(xg + 0.025, 0.02)) %>%
    ungroup()
...
ggplot(xg_sample_for_plot %>% filter(name %in% c("min_max_goal", "min_max_xg_soph", "min_max_xg_naive")) %>% pivot_wider(names_from = name, values_from = goals), aes(x = shots)) +
  ...
  transition_reveal(reveal_time) +
  view_follow(fixed_x = TRUE, fixed_y = TRUE) +
  labs(title = glue::glue("<b>Relative, one-sided error (95% confidence interval) for Goals, </b><b style = 'color:#DC2228'>Sophisticated</b> and </b><b> and</b><b style = 'color:#91bfdb'>Naive</b><b> xG</b>"), 
...
I have introduced a framework to roughly quantify the varying quality of xG models. By comparing their behavior in analyzing shots from almost 300 team seasons to a hypothetical, true xG model I find that the more sophisticated Statsbomb model outperforms a simpler model represented by Understat data. 

I further investigate if any of the models show a bias in analyzing shots of teams with either high or low shot volume. I find that naive xG models tend to underestimate expected goals for teams with high shot volume and overestimates xG for teams with low shot volume. I do not find this effect for the more sophisticated Statsbomb model that includes additional features like shot freeze frames, goalkeeper positioning and shot impact height.
...
A possible explanation for these observations would be that top teams with higher shot volume are more careful about choosing their shooting opportunities and when in doubt (congested 18 yard box or awkward shot height) recycle the ball instead of pulling the trigger.
