...
```{r Get StatsBomb Data Initial, message=FALSE, warning=FALSE, echo=FALSE, results = 'hide', cache = TRUE}
competition <- 11
...
pass_areas <- passes_clean %>%
  mutate(location.y = pitch_height - location.y ) %>% # Flip y-coordinates
  mutate(pass.end_location.y = pitch_height - pass.end_location.y) %>% # Flip y-coordinates
...
pass_areas <- passes_clean %>%
  mutate(location.y = pitch_height - location.y ) %>% # Flip y-coordinates
  mutate(carry.end_location.y = pitch_height - carry.end_location.y) %>% # Flip y-coordinates
  mutate(area_origin_x = ceiling(location.x * num_zones_width / pitch_width)) %>%
  mutate(area_origin_y = ceiling(location.y * num_zones_height / pitch_height)) %>%
  mutate(area_origin = (area_origin_x - 1) * num_zones_height + area_origin_y) %>%
